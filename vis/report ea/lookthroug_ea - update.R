...
## --- 3) Nodes (3 columns) ----------------------------------------------------
left_node <- list(list(
  id="S1M_src", name="Households (S1M)", column=0, color="#D9D9D9",
...
highchart() %>%
  ...
  hc_title(text = "Households’ financial assets stocks, direct vs via financial sector ") %>%
  hc_subtitle(text = "EA20, 2025 Q1 (Fx7). Yellow = holdings via fin. sector transformation; Grey = direct holdings") %>%
