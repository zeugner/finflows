...
# S1M assets
#as1m=aa[.EA20.S1M+S1.S12+S12K+S12Q+S12O+S12R+S124+S11+S13+S1M+S1.LE._T.2024q4.W2+WRL_REST]
as1t=aa[.EA20+DE+AT+FR+NL+IE+ES+BE+SI+PT+FI+IT+LT+CY+LU+LV+SK+EE+GR+HR+MT+CZ+HU+SE+DK+RO+BG.S1M+S1.S12+S11+S13+S1M+S1.LE._T.2024q4.W2+WRL_REST]
...
# S12 assets
#as12s=aa[.EA20.S1+S12+S12K+S12R+S12Q+S12O+S124.S12K+S12Q+S12O+S124S12R+S11+S13+S1M+S1.LE._T.2024q4.W2+WRL_REST]
as12s=aa[Fx7.EA20.S1+S12.S12K+S12Q+S12O+S124+S11+S13+S1M+S1.LE._T.2024q4.W2+WRL_REST]
...
mid_nodes <- lapply(seq_len(nrow(mid_totals)), function(i) {
  ...
  list(
    id     = m$mid_id,
    name   = sprintf("%s — %.1f%% of HH assets", m$mid_label, 100 * m$share),
    column = 1,
    color  = m$color,
    dataLabels = list(style = list(color="black", textOutline="none"))
  )
...
left_node <- list(list(
  id="S1M_src", name="Households (HH)", column=0, color="#D9D9D9",
  dataLabels=list(style=list(color="black", textOutline="none"))
))
...
highchart() %>%
  ...
  hc_title(text = " ") %>%
  hc_subtitle(text = "EA20, financial assets. Yellow = holdings via financial sector; Grey = direct holdings") %>%
  hc_caption(text = " ") %>%
