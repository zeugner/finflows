...
# Get F3 values for calculating the threshold
f3_values = aall[F3..S1.S0.LE._T.2023q4]

# Calculate threshold as 1% of F3 for each country
threshold = abs(f3_values) * 0.01

# Check
check6a_assets = aall[F3..S1.S0.LE._T.2023q4] - 
  (aall[F3S..S1.S0.LE._T.2023q4] + aall[F3L..S1.S0.LE._T.2023q4])

# Apply fifelse with dynamic threshold
print("F3 Assets check results:")
fifelse(abs(check6a_assets) > threshold, yes='nok', no='ok', na=NA)


# Extract problematic values for F3 assets
values_f3_assets <- check6a_assets$"_.obs_value"
threshold_values <- threshold$"_.obs_value"
countries <- check6a_assets$REF_AREA

# Use the same threshold logic for displaying problematic values
result_f3_assets <- fifelse(abs(values_f3_assets) > threshold_values, 
