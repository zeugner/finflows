...
loaddata=!file.exists('data/govf5src.rds')
...
if (loaddata) {
  ...
  saveRDS(list(gov5raw=gov5raw,goviipq=goviipq,goviipa=goviipa,govqsa=govqsa, govqsl=govqsl, xrraw=xrraw, govcp=govcp),file='data/govf5src.rds')
} else {
  lll=readRDS('data/govf5src.rds'); gov5raw=lll$gov5; goviipq=lll$goviipq; goviipa=lll$goviipa; govqsa=lll$govqsa; govqsl=lll$govqsl; govcp=lll$govcp; xrraw=lll$xrraw; rm(lll)
}
...
#gov5[A_LE.IT...NCO.2022q4]
gov5[..S13..NCO., onlyna=TRUE] = gov5[..S0..NCO.]-gov5[..S0..CO.]
#gov5[A_LE.IT...NCO.2022q4]


gov5[A_LE.MT+GR+RO+BG.S2.S1314.NCO.,onlyna=TRUE] = 0
# temp=gov5[A_LE..S2..NCO.2022q4]; names(dimnames(temp))[2]='bop_item'
# str(temp)
# temp[,dimnames(goviip)$bop_item] = goviip[.A_LE..y2022q4]
# temp
...
saveRDS(gov_equity,'data/gov_equity.rds')
