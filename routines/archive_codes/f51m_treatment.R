...
if (!dir.exists(file.path(getwd(),'data'))) {
  data_dir = '\\\\s-jrciprnacl01p-cifs-ipsc.jrc.it/ECOFIN/FinFlows/githubrepo/finflows/data/'
} else {
...
aall=readRDS(file.path(data_dir,'intermediate_domestic_data_files/aall_equity_bilateral_imf_gov.rds')); gc()
...
ecb_capital_md3=readRDS(file.path(data_dir,'ecb_capital_md3.rds')); gc()

aall[F519..S121.S2.LE._O.,onlyna=TRUE]<-ecb_capital_md3

# aall[F519.BG.S121.S2.LE._O.2024q4]<-4.0
# aall[F519.DK.S121.S2.LE._O.2024q4]<-7.2
 aall[F519.DE.S121.S2.LE._O.2024q4]
# aall[F519.IE.S121.S2.LE._O.2024q4]<-192.8
# aall[F519.LU.S121.S2.LE._O.2024q4]<-32.2
# aall[F519.MT.S121.S2.LE._O.2024q4]<-11.4

# aall["F519..S121.S2.LE._O.2019q4:2024q3",onlyna=TRUE]<-aall["F519..S121.S2.LE._O.2024q4"]
gc()


### FILL COUNTERPART S1 (DOMESTIC ECONOMY) AS RESIDUAL
...
### FDI liabilities for some countries for S1M are zero from QSA but actually are not so according to BOP IIP

aall["F51M..S2.S1M.LE._D."]<-0
aall["F51M..S2+S0+S1.S121.LE..2022q4"]

aall["....._S."]=NA;gc()
### unflagging necessary to get the "obs_status" away
aall=unflag(aall)
aall_S=as.data.table(aall,.simple=TRUE); gc()
aallcast=dcast(aall_S, ... ~ FUNCTIONAL_CAT, id.vars=1:8, value.var = 'obs_value'); gc()
...
aallcast2=copy(aallcast[,intersect(c(colnames(aall_S),'_S'),colnames(aallcast)),with=FALSE]);gc()
colnames(aallcast2)[NCOL(aallcast2)]='obs_value';gc()
aallcast2[,FUNCTIONAL_CAT:='_S'];gc()
aall_S=rbind(aall_S,aallcast2,fill=TRUE);gc()     
aall_M=as.md3(aall_S);gc()
aall=copy(aall_M)
...
#### line initiated by Stefan, do not know what we were supposed to do here...
#aall[F5+F51+F51M+F511+F512+F519..S1.S2.LE._O.,onlyna=TRUE]=
...
# aall["F51M..S1+S11+S12T+S2+S1M.S1M.LE._T.2023q4"]  # CHECK: View after
...
aall[F51M..S12T.S11+S1+S12T+S12O.LE._S.2023q4]  # CHECK: View before
...
# 
# aall[.AT.S12.S11.LE..2023q4]
# ################## SECOND STEP: OFI TO MFI HOLDINGS #################
# # Calculate OFI holdings of MFI liabilities as residual
# # Formula: OFI assets - OFI holding of NFC - OFI self-holdings
# # Economic assumption: OFI doesn't hold S13 (government) or S1M (household) unlisted equity
# 
# aall[F51M..S12O.S1+S11+S12O+S12T.LE._S.2023q4]  # CHECK: View components before
...
# aall[F51M..S12O.S12T.LE._T.2023q4]  # CHECK: Verify calculation

#aall[F51M..S12O.S12T.LE._S.2023q4]
...
source("path_to_be_changed/F51M_IT.R")
...
# aall[F51M..S1M.S12+S12O+S121+S12T+S12K.LE._T.2023q4]  # CHECK: View HH holdings in financial sector
...
### Set banks and central banks holdings by households
...
### NFC HOLDINGS OF MFI LIABILITIES
# Calculate as residual: MFI liabilities - MFI self - OFI holdings in MFI
# aall[F51M..S1.S12K+S121+S12T.LE._T.2022q4]  # CHECK: View data availability
# aall[F51M..S1.S12T.LE._T.2022q4]  # CHECK: View S1 holdings of S12T

### COMPUTE F51M.S11.S12T AS RESIDUAL
# aall[F51M..S11+S1+S12T+S12O.S12T.LE._T.2023q4]  # CHECK: View before calculation

### To get F51M.S1.S12T, follow these steps:
...
aall[F51..S0.S121.LE._S.2022q4]
 
aall[F51M..S0.S121.LE._S.2024]
...
aall[F51M..S2.S121...] <-aall[F51M..S2.S12K...]-aall[F51M..S2.S12T...]
...
aall["F51M..S11.S12T.LE._S.2022q4"]
...
# aall[F51M.IT+DE.S1M+S1+S12O+S12T.S11.LE._T.2022q4]  # CHECK: Verify after
...
# Formula: S1 total - MFI self - NFC - OFI
# Note: Issue for France - no F51 info from BSI
...
###  Insurance Holdings of OFI
# TO BE CHECKED WITH STEFAN 
###aall[F51M..S12Q.S12O..., onlyna=TRUE] = aall[F51M..S12.S12...] - zerofiller(aall[F51M..S121.S12O...])- zerofiller(aall[F51M..S12T.S12O...])- zerofiller(aall[F51M..S124.S12O...])- zerofiller(aall[F51M..S121.S12O...])- zerofiller(aall[F51M..S121.S12O...])- zerofiller(aall[F51M..S121.S12O...])
  ######sum(aall[F51M..S121+S12T+S124+S12Q+S12O.S121+S12T+S124+S12Q+S12O...], na.rm=TRUE)


saveRDS(aall,'data/intermediate_domestic_data_files/aall_f51m.rds')
